...
import { Search, Palette, Grid, Box, Circle, Type, Layout, BarChart, FileText } from 'lucide-react';
import { CATEGORIES, COMPONENTS } from '../constants/components';
...
const Sidebar = ({ activeCategory, onSelectCategory, searchQuery, setSearchQuery, isOpen, onClose }) => {
    ...
    return (
        <>
            {/* Mobile Overlay */}
            <div
                className={`fixed bg-brand-dark inset-0   backdrop-blur-sm z-40 transition-opacity duration-300 md:hidden
          ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
            ...
            {/* Sidebar Container */}
            <aside
                className={`fixed md:sticky top-0 md:top-20 left-0 h-full md:h-[calc(100vh-5rem)] w-[300px] bg-brand-dark/90 backdrop-blur-xl border-r border-white/5 z-50 transition-transform duration-300 transform 
